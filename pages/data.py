...
#balancete_pivot_test.rename(columns={'competencia':'COMPETENCIA'},inplace=True)
...
ativo_total = str('1'+zeros)[:22]
exigivel_operacional = str('2.01'+zeros)[:22]
exigivel_contingencial = str('2.02'+zeros)[:22]
patrimonio_social =  str('2.03'+zeros)[:22]
patrimonio_cobertura =  str('2.03.01'+zeros)[:22]
provisoes_matematicas = str('2.03.01.01'+zeros)[:22]
provisoes_constituir =  str('2.03.01.01.03'+zeros)[:22]
...
inpc_ibge = 'https://apisidra.ibge.gov.br/values/t/1736/n1/all/v/44/p/all/d/v44%202'
...
inpc['Mês (Código)'] = pd.to_datetime(inpc['Mês (Código)'], format='%Y%m')
...
def update_table(mes,plano,mes_anterior):
    ...
    ipca_filtro2 = mes #pd.to_datetime(mes) - relativedelta(months = 1) if plano == 'BD-01' else mes
    ...
    [  'Patrimônio Líquido de Cobertura',
        ...
        'Resultado',
        f'Inflação do período {periodo} (t)', #f'Inflação do período {periodo} (t-1)' if plano =='BD-01' else 
    ...
    [   montante(patrimonio_cobertura,mes,plano),
        ...
        "{:.2f}%".format(inpc[inpc['Mês (Código)'] == ipca_filtro2].Valor.values[0]).replace(".",","),
